exports['uriBinder() Bind ["file:///a.nbtdoc","minecraft/foo.nbtdoc","minecraft/bar.nbtdoc","minecraft/qux.mcfunction"] 1'] = {
  "nbtdoc": {
    "minecraft/foo": {
      "category": "nbtdoc",
      "identifier": "minecraft/foo",
      "subcategory": "module",
      "implementation": [
        {
          "uri": "file:///root/minecraft/foo.nbtdoc",
          "range": {
            "start": 0,
            "end": 0
          }
        }
      ]
    },
    "minecraft/bar": {
      "category": "nbtdoc",
      "identifier": "minecraft/bar",
      "subcategory": "module",
      "implementation": [
        {
          "uri": "file:///root/minecraft/bar.nbtdoc",
          "range": {
            "start": 0,
            "end": 0
          }
        }
      ]
    }
  }
}

exports['uriBinder() Bind ["minecraft/mod.nbtdoc","minecraft/foo/mod.nbtdoc"] 1'] = {
  "nbtdoc": {
    "minecraft": {
      "category": "nbtdoc",
      "identifier": "minecraft",
      "subcategory": "module",
      "implementation": [
        {
          "uri": "file:///root/minecraft/mod.nbtdoc",
          "range": {
            "start": 0,
            "end": 0
          }
        }
      ]
    },
    "minecraft/foo": {
      "category": "nbtdoc",
      "identifier": "minecraft/foo",
      "subcategory": "module",
      "implementation": [
        {
          "uri": "file:///root/minecraft/foo/mod.nbtdoc",
          "range": {
            "start": 0,
            "end": 0
          }
        }
      ]
    }
  }
}

exports['uriBinder() Bind ["qux.nbtdoc","nbtdoc/foo.nbtdoc","nbtdoc/minecraft/bar.nbtdoc"] 1'] = {
  "nbtdoc": {
    "foo": {
      "category": "nbtdoc",
      "identifier": "foo",
      "subcategory": "module",
      "implementation": [
        {
          "uri": "file:///root/nbtdoc/foo.nbtdoc",
          "range": {
            "start": 0,
            "end": 0
          }
        }
      ]
    },
    "minecraft/bar": {
      "category": "nbtdoc",
      "identifier": "minecraft/bar",
      "subcategory": "module",
      "implementation": [
        {
          "uri": "file:///root/nbtdoc/minecraft/bar.nbtdoc",
          "range": {
            "start": 0,
            "end": 0
          }
        }
      ]
    }
  }
}

exports['uriBinder() Bind ["qux.nbtdoc","nbtdoc/mod.nbtdoc","nbtdoc/foo.nbtdoc","nbtdoc/minecraft/bar.nbtdoc"] 1'] = {
  "nbtdoc": {
    "qux": {
      "category": "nbtdoc",
      "identifier": "qux",
      "subcategory": "module",
      "implementation": [
        {
          "uri": "file:///root/qux.nbtdoc",
          "range": {
            "start": 0,
            "end": 0
          }
        }
      ]
    },
    "nbtdoc": {
      "category": "nbtdoc",
      "identifier": "nbtdoc",
      "subcategory": "module",
      "implementation": [
        {
          "uri": "file:///root/nbtdoc/mod.nbtdoc",
          "range": {
            "start": 0,
            "end": 0
          }
        }
      ]
    },
    "nbtdoc/foo": {
      "category": "nbtdoc",
      "identifier": "nbtdoc/foo",
      "subcategory": "module",
      "implementation": [
        {
          "uri": "file:///root/nbtdoc/foo.nbtdoc",
          "range": {
            "start": 0,
            "end": 0
          }
        }
      ]
    },
    "nbtdoc/minecraft/bar": {
      "category": "nbtdoc",
      "identifier": "nbtdoc/minecraft/bar",
      "subcategory": "module",
      "implementation": [
        {
          "uri": "file:///root/nbtdoc/minecraft/bar.nbtdoc",
          "range": {
            "start": 0,
            "end": 0
          }
        }
      ]
    }
  }
}
