...
export function onDidChangeTextDocument({ uri, info, roots, version, contentChanges, config, cacheFile, commandTree, vanillaData }: { uri: Uri, info: FunctionInfo, version: number, contentChanges: TextDocumentContentChangeEvent[], config: Config, cacheFile: CacheFile, roots: Uri[], commandTree?: CommandTree, vanillaData?: VanillaData }) {
    ...
    for (const change of contentChanges) {
        ...
        lineDelta = 0
        nodeChange = { nodeStart: 0, nodeStop: info.nodes.length - 1 }
        break
    }
    nodeChange = nodeChange ?? { nodeStart: 0, nodeStop: info.nodes.length - 1 }
    ...
    parseStrings(
        info.document,
        nodeChange.lineStart ? info.document.offsetAt(Position.create(nodeChange.lineStart, 0)) : undefined,
        nodeChange.lineStop ? info.document.offsetAt(Position.create(nodeChange.lineStop + lineDelta, Infinity)) : undefined,
