...
describe('NumberArgumentParser Tests', () => {
    describe('parse() Tests', () => {
        ...
        it('Should return error when the number is larger than max', () => {
            ...
            assert(data === 2333)
            assert.deepStrictEqual(errors, [
                new ParsingError({ start: 0, end: 4 }, 'expected a number smaller than 6 but got 2333')
            ])
        })
        it('Should return error when the number is smaller than min', () => {
            ...
            assert(data === 2333)
            assert.deepStrictEqual(errors, [
                new ParsingError({ start: 0, end: 4 }, 'expected a number larger than 114514 but got 2333')
            ])
        })
    })
