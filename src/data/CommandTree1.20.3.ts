...
import { AlwaysValidates, ArgumentParserResult, ErrorCode, ParsingContext, ParsingError, Switchable, combineArgumentParserResult } from '../types'
...
export const CommandTree: ICommandTree = {
    ...
    commands: {
        ...
        gamerule: {
            ...
            children: {
                boolRuleName: {
                    parser: new LiteralArgumentParser('announceAdvancements', 'blockExplosionDropDecay', 'commandBlockOutput', 'disableElytraMovementCheck', 'disableRaids', 'doDaylightCycle', 'doEntityDrops', 'doFireTick', 'doImmediateRespawn', 'doInsomnia', 'doLimitedCrafting', 'doMobLoot', 'doMobSpawning', 'doPatrolSpawning', 'doTileDrops', 'doTraderSpawning', 'doVinesSpread', 'doWardenSpawning', 'doWeatherCycle', 'drowningDamage', 'enderPearlsVanishOnDeath', 'fallDamage', 'fireDamage', 'forgiveDeadPlayers', 'freezeDamage', 'globalSoundEvents', 'keepInventory', 'lavaSourceConversion', 'logAdminCommands', 'mobExplosionDropDecay', 'mobGriefing', 'naturalRegeneration', 'reducedDebugInfo', 'projectilesCanBreakBlocks', 'sendCommandFeedback', 'showDeathMessages', 'spectatorsGenerateChunks', 'tntExplosionDropDecay', 'universalAnger', 'waterSourceConversion'),
                    executable: true,
                    children: {
                        value: {
                            template: 'templates.boolean',
        ...
        random: {
            parser: new LiteralArgumentParser('random'),
            children: {
                value_roll: {
                    parser: new LiteralArgumentParser('value', 'roll'),
                    children: {
                        range: {
                            ...
                            children: {
                                sequence: {
                                    ...
                                    parser: new IdentityArgumentParser('sequence', false, false, true),
                                    executable: true
                                }
                            }
                        }
                ...
                reset: {
                    ...
                    children: {
                        sequences: {
                            // TODO: provide autocomplete for vanilla sequences, *, as well as custom sequences
                            // i'll keep this as an anonymous class to signal that this has to be done (or not)
                            parser: new class extends ArgumentParser<string> {
                                static identity: string = 'Sequences'
                                readonly identity: string = 'sequences'
                                ...
                                parse(reader: StringReader, ctx: ParsingContext): ArgumentParserResult<string> {
                                    ...
                                    if (reader.peek() === '*') {
                                        ...
                                        const idResult = new IdentityArgumentParser('sequence', false, false, true).parse(reader, ctx)
                                        const id = idResult.data as IdentityNode

                                        combineArgumentParserResult(ans, idResult)
                                        ans.data = id.toString()
                                    }

                                    return ans
                                }

                                // hacky.... we'll clean it up eventually
                                toHint(name: string, optional: boolean): string {
                                    return `[<*|${this.identity}: ${locale('argument-type.identity')}>]`
                                }
                            },
                            executable: true,
                            children: {
    ...
    templates: {
        ...
        color: {
            parser: new LiteralArgumentParser('black', 'dark_blue', 'dark_green', 'dark_aqua', 'dark_red', 'dark_purple', 'gold', 'gray', 'dark_gray', 'blue', 'green', 'aqua', 'red', 'light_purple', 'yellow', 'white', 'reset')
        },
        execute_if_unless: {
