...
describe('ResourceLocationNode', () => {
	describe('toString()', () => {
		const suites: { node: Partial<ResourceLocationNode>, type: 'origin' | 'full' | 'short' | undefined, includesTagPrefix?: boolean, expected: string }[] = [
			...
			{ node: { isTag: true, path: ['foo', 'bar'] }, type: 'origin', includesTagPrefix: true, expected: '#foo/bar' },
			{ node: { isTag: true, path: ['foo', 'bar'] }, type: 'full', includesTagPrefix: true, expected: '#minecraft:foo/bar' },
			{ node: { isTag: true, path: ['foo', 'bar'] }, type: 'short', includesTagPrefix: true, expected: '#foo/bar' },
		]
		for (const { node, type, includesTagPrefix, expected } of suites) {
			it(`It should convert to "${showWhitespaceGlyph(expected)}"`, () => {
				const actual = ResourceLocationNode.toString(node as ResourceLocationNode, type, includesTagPrefix)
