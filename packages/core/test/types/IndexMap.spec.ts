...
describe('IndexMap', () => {
	...
	describe('Position Transformation', () => {
		const suites: { desc: string, map: IndexMap, toInnerCases: { input: number, expected: number | null }[], toOuterCases: { input: number, expected: number | null }[] }[] = [
			{
				/*
		...
		for (const { desc, map, toInnerCases, toOuterCases } of suites) {
			describe(desc, () => {
