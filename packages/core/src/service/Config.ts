...
export interface EnvConfig {
	 ...
	 * - An absolute file path to a spyglass project (should have `spyglass.json` and/or `pack.mcmeta` in the root) folder or `.zip`/`.tar.gz` archive. // TODO: Support file paths relative to the project root.
	 ...
	 *   where the root or folder path will be considered a spyglass project (should have `spyglass.json` and/or `pack.mcmeta` in the root).
	 *
	 ...
	 * - A raw link to a Git hosted `zip`/`tar.gz` spyglass project (should have `spyglass.json` and/or `pack.mcmeta` in the root), supports GitHub, GitLab & Gitea.
	 *
	 ...
	 * - A direct http(s) link to a `zip`/`tar.gz` spyglass project (should have `spyglass.json` and/or `pack.mcmeta` in the root), caching behavior unknown. // TODO: determine caching behavior
	 */
	dependencies: string[]
	/**
