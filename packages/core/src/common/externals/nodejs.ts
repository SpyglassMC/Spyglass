...
import type { Externals, FsLocation, FsWatcher } from '.'
import { promisifyAsyncIterable, Uri } from '../util'
import type { ExternalDownloader, ExternalDownloaderOptions, RemoteUriString } from './downloader'
...
export const NodeJsExternals: Externals = {
	...
	fs: {
		...
		async getAllFiles(location) {
			return (await globby(NodeJsExternals.uri.toPath(location) + '**/*', { absolute: true, dot: true })).map(NodeJsExternals.uri.fromPath)
		},
		...
		async showFile(location): Promise<void> {
			...
			return void await execFile(command, [NodeJsExternals.uri.toPath(location)])
		},
		stat(location) {
		...
		watch(location) {
			return new ChokidarWrapper(chokidar.watch(NodeJsExternals.uri.toPath(location)))
		},
