...
import url from 'url'
import { CommandArgumentTestSuites } from './_suites.mjs'

// Generates test files for each command argument parser with test suites from `./suites.mts`.
...
if (ShouldGenerate) {
	for (const parser of Object.keys(CommandArgumentTestSuites)) {
		fs.writeFileSync(
			path.join(url.fileURLToPath(new URL('.', import.meta.url)), `${parser.replace(/[:_](\w)/g, (_, c) => c.toUpperCase())}.generated.spec.mts`),
			template(parser),
			{ encoding: 'utf-8' }
		)
	}
