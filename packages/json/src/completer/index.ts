...
export const object: Completer<JsonObjectNode> = core.completer.record<
	JsonStringNode,
	JsonNode,
	JsonObjectNode
>({
...
function objectCompletion(
	range: RangeLike,
	node: DeepReadonly<JsonObjectNode>,
	expectation: DeepReadonly<JsonObjectExpectation>,
	...
	if (expectation.fields) {
		...
		return expectation.keys.flatMap((e) =>
			stringCompletion(range, e, ctx).map((c) => ({
				...c,
				...(insertValue
...
function fieldCompletion(
	range: RangeLike,
	field: DeepReadonly<
		Exclude<JsonObjectExpectation['fields'], undefined>[number]
	>,
	insertValue: boolean,
	insertComma: boolean,
): CompletionItem {
...
function valueCompletion(
	range: RangeLike,
	expectation: readonly DeepReadonly<JsonExpectation>[],
	ctx: CompleterContext,
): CompletionItem[] {
	return unique(
		expectation.flatMap((e) => {
			switch (e.type) {
				case 'json:object':
				case 'json:array':
...
function stringCompletion(
	range: RangeLike,
	expectation: DeepReadonly<JsonStringExpectation>,
	ctx: CompleterContext,
): CompletionItem[] {
	if (Array.isArray(expectation.pool)) {
		return expectation.pool.map((v) =>
			CompletionItem.create(v, range, {
