...
const number =
	(type: 'integer' | 'float') =>
	(min: number | undefined, max: number | undefined): JsonChecker => {
		return (node, ctx) => {
			...
			if (
				...
				ctx.err.report(
					localize('expected', localize('number.between', min, max)),
					node,
				)
			} else if (min !== undefined && node.value < min) {
				ctx.err.report(
					localize('expected', localize('number.>=', min)),
					node,
				)
			} else if (max !== undefined && node.value > max) {
				ctx.err.report(
					localize('expected', localize('number.<=', max)),
					node,
				)
			}
		}
