...
import type { NbtByteNode, NbtNumberNode, NbtPrimitiveNode } from '../node/index.mjs'
import { localizeTag } from '../util.mjs'
...
const NumeralPatterns: (
	...
	{ pattern: RegExp, type: NbtNumberNode['type'], hasSuffix: boolean, group: Group.IntegerAlike, min: number, max: number } |
	{ pattern: RegExp, type: NbtNumberNode['type'], hasSuffix: boolean, group: Group.LongAlike, min: bigint, max: bigint }
)[] =
	[
		{ pattern: /^[-+]?(?:0|[1-9][0-9]*)b$/i, type: 'nbt:byte', hasSuffix: true, group: Group.IntegerAlike, min: -128, max: 127 },
		{ pattern: /^[-+]?(?:0|[1-9][0-9]*)s$/i, type: 'nbt:short', hasSuffix: true, group: Group.IntegerAlike, min: -32768, max: 32767 },
		{ pattern: /^[-+]?(?:0|[1-9][0-9]*)$/, type: 'nbt:int', hasSuffix: false, group: Group.IntegerAlike, min: -2147483648, max: 2147483647 },
		{ pattern: /^[-+]?(?:0|[1-9][0-9]*)l$/i, type: 'nbt:long', hasSuffix: true, group: Group.LongAlike, min: -9223372036854775808n, max: 9223372036854775807n },
...
export const primitive: core.InfallibleParser<NbtPrimitiveNode> = (src: core.Source, ctx: core.ParserContext) => {
	...
	for (const e of NumeralPatterns) {
		if (e.pattern.test(unquotedResult.value)) {
			...
			const numeralParser: core.InfallibleParser<core.FloatNode | core.IntegerNode | core.LongNode> = e.group === Group.IntegerAlike
