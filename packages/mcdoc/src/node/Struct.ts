...
import type { DispatcherData, DispatcherType, McdocType, NumericRange, ReferenceType } from '../type'
...
export namespace RegistryIndexNode {
	...
	export function toData(node: RegistryIndexNode): DispatcherData {
		...
		return {
			...
			index: [{
				kind: 'dynamic',
				accessor: node.path.map(v => LiteralToken.is('super')(v) ? { keyword: 'parent' } : v.value),
			}],
		}
	}
}
...
export namespace CompoundFieldNode {
	export function is(obj: object | undefined): obj is CompoundFieldNode {
		return (obj as CompoundFieldNode | undefined)?.type === 'mcdoc:struct/field'
	}
 ...
 * 
 * [doc]: https://github.com/Yurihaia/mcdoc-rs/blob/master/docs/format.md#field-type
 */
export type CompoundFieldTypeNode = AstNode & {
	type: 'mcdoc:struct/field/type',
	typeKind: string,
} & ({
	typeKind: 'boolean',
} | {
...
export namespace CompoundFieldTypeNode {
	export function is(obj: object): obj is CompoundFieldTypeNode {
		return (obj as CompoundFieldTypeNode).type === 'mcdoc:struct/field/type'
	}

	/**
	 * @param symbol If `node.typeKind === 'path'`, this parameter will be used to fill in the {@link SymbolData}'s `symbol` property.
	 */
	export async function toType(node: CompoundFieldTypeNode, resolveIdentPath: ResolveIdentPathFunc): Promise<McdocType> {
...
export namespace IntRangeNode {
	export function is(obj: object): obj is IntRangeNode {
		return (obj as IntRangeNode).type === 'mcdoc:int_range'
	}
...
export namespace FloatRangeNode {
	export function is(obj: object): obj is FloatRangeNode {
		return (obj as FloatRangeNode).type === 'mcdoc:float_range'
	}
