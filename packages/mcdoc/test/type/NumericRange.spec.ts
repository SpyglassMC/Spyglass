import { describe, it } from 'node:test'
import { NumericRange } from '../../lib/type/index.js'

describe('NumericRange', () => {
	describe('intersect()', () => {
		const suites: [NumericRange, NumericRange][] = [
			[{ kind: 0b00, min: 2, max: 4 }, { kind: 0b00, min: 3, max: 7 }],
			[{ kind: 0b00, min: 2, max: 3 }, { kind: 0b00, min: 3, max: 7 }],
			[{ kind: 0b00, min: 2, max: 4 }, { kind: 0b00, min: 5, max: 7 }],
			[{ kind: 0b10, min: 2, max: 7 }, { kind: 0b01, min: 3, max: 5 }],
			[{ kind: 0b11, min: 2, max: 5 }, { kind: 0b10, min: 5, max: 6 }],
			[{ kind: 0b00, min: 2, max: 5 }, { kind: 0b01, min: 4, max: 5 }],
			[{ kind: 0b00, min: 2, max: 9 }, { kind: 0b10, min: 4, max: 5 }],
			[{ kind: 0b00, min: 2, max: 9 }, { kind: 0b11, min: 2, max: 9 }],
			[{ kind: 0b11, min: 2, max: 9 }, { kind: 0b00, min: 2, max: 9 }],
			[{ kind: 0b00, max: 9 }, { kind: 0b00, min: 4 }],
			[{ kind: 0b01, max: 9 }, { kind: 0b00, max: 8 }],
			[{ kind: 0b00 }, { kind: 0b10, min: 2 }],
			[{ kind: 0b00 }, { kind: 0b00 }],
			[{ kind: 0b00, min: 2, max: 4 }, { kind: 0b00, min: 3n, max: 7n }],
			[{ kind: 0b00, min: 2, max: 3 }, { kind: 0b00, min: 3n, max: 7n }],
			[{ kind: 0b00, min: 2, max: 4 }, { kind: 0b00, min: 5n, max: 7n }],
			[{ kind: 0b10, min: 2, max: 7 }, { kind: 0b01, min: 3n, max: 5n }],
			[{ kind: 0b11, min: 2, max: 5 }, { kind: 0b10, min: 5n, max: 6n }],
			[{ kind: 0b00, min: 2, max: 5 }, { kind: 0b01, min: 4n, max: 5n }],
			[{ kind: 0b00, min: 2, max: 9 }, { kind: 0b10, min: 4n, max: 5n }],
			[{ kind: 0b00, min: 2, max: 9 }, { kind: 0b11, min: 2n, max: 9n }],
			[{ kind: 0b11, min: 2, max: 9 }, { kind: 0b00, min: 2n, max: 9n }],
			[{ kind: 0b00, max: 9 }, { kind: 0b00, min: 4n }],
			[{ kind: 0b01, max: 9 }, { kind: 0b00, max: 8n }],
			[{ kind: 0b00 }, { kind: 0b10, min: 2n }],
		]
		for (const [a, b] of suites) {
			it(`${NumericRange.toString(a)} & ${NumericRange.toString(b)}`, (t) => {
				const intersection = NumericRange.intersect(a, b)
				t.assert.snapshot(NumericRange.toString(intersection))
			})
		}
	})
})
