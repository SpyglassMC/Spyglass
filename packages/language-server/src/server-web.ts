...
import * as ls from 'vscode-languageserver/browser.js'
import type {
...
const cacheRoot = 'file:///cache/spyglassmc/'

const messageReader = new ls.BrowserMessageReader(self)
const messageWriter = new ls.BrowserMessageWriter(self)
const connection = ls.createConnection(messageReader, messageWriter)
...
const externals = BrowserExternals
const logger: core.Logger = {
	error: (msg: any, ...args: any[]): void => connection.console.error(`${msg} ${args.join(' ')}`),
	info: (msg: any, ...args: any[]): void => connection.console.info(`${msg} ${args.join(' ')}`),
	log: (msg: any, ...args: any[]): void => connection.console.log(`${msg} ${args.join(' ')}`),
	warn: (msg: any, ...args: any[]): void => connection.console.warn(`${msg} ${args.join(' ')}`),
}
