...
export async function generate(project_path: string, generated_path: string, args: Args, doc_file: walk.Item, service: Service, logger: Logger) {
	const symbols = []
		...
		if (node.children[0]) {
			...
			function flattenChild(
						...
						if (threeBack?.type === 'mcdoc:struct') {
									...
									case 'mcdoc:struct/field/pair': {
										...
										if (key) {
											if (foundRoot) {
												...
												/* @ts-ignore */
												const actualRoot = sixBack?.parent?.parent?.parent?.children?.find(child => child.type === 'mcdoc:identifier')?.value
									...
									case 'mcdoc:type/union': {
										switch (fourBack?.parent?.type) {
											case 'mcdoc:struct/field/spread': {
												/* @ts-ignore */
												const root = fourBack?.parent?.parent?.parent?.children?.find(child => child.type === 'mcdoc:identifier')?.value
											...
											case 'mcdoc:dispatch_statement': {
												...
												if (indexGuess === '45') {
													container = `__dispatch.${registry}.${key}.__union.__struct_${self.split('][').slice(-4)[0] }` 
												} else {
													container = `__dispatch.${registry}.${key}.__union.__struct_${indexGuess}`
												}
											} break;
												...
												if (foundRoot) {
													...
													switch (sevenBack?.parent?.type) {
														case 'mcdoc:type/list': {
															...
															// Credits husk
															/* @ts-ignore */
															const actualRoot = nineBack?.parent?.parent?.parent?.parent?.children?.find(child => child.type === 'mcdoc:identifier')?.value
															if (actualRoot) {
																container = `${actualRoot}.__struct_list.${parentKey}.__struct_list.${key}.__struct_list`
															} else {
														...
														case 'mcdoc:type/union': {
															/* @ts-ignore */
															const actualRoot = sevenBack.parent?.parent?.children?.find(child => child.type === 'mcdoc:identifier')?.value

															// block state definition, this is fragile

															if (actualRoot) {
																container = `${actualRoot}.__struct_union.${key}.__struct_list`
															} else {
										...
										if (!container) { // yes
											switch (fourBack?.parent?.type) {
												...
												case 'mcdoc:type/union': {
													...
													container = `${root}.${parentKey}.__union_list.__struct`
												} break;
												case 'mcdoc:type/list': {
													// texture meta
													/* @ts-ignore */
													const parentKey = fourBack?.parent.parent?.children?.find(child => child.type === 'mcdoc:identifier')?.value

													/* @ts-ignore */
													const rootKey = fourBack?.parent?.parent?.parent?.parent?.parent?.children?.find(child => child.type === 'mcdoc:identifier')?.value

													/* @ts-ignore */
													const root = fourBack?.parent?.parent?.parent?.parent?.parent?.parent?.parent?.children?.find(child => child.type === 'mcdoc:identifier')?.value

													container = `${root}.${rootKey}.${parentKey}.__union_list.__struct`
												} break;
											}
										}
									} break;
							...
							switch (threeBack?.type) {
								case 'mcdoc:enum': {
									...
									if (root) {
										...
										if (root) {
											...
											/* @ts-ignore */
											const rootKey = threeBack?.parent?.parent?.parent?.parent?.children?.find(child => child.type === 'mcdoc:identifier')?.value
											/* @ts-ignore */
											const root = threeBack?.parent?.parent?.parent?.parent?.parent?.parent?.children?.find(child => child.type === 'mcdoc:identifier')?.value
								...
								case 'mcdoc:struct/field/pair': {
									...
									/* @ts-ignore */
									const root = threeBack.parent?.parent?.children?.find(child => child.type === 'mcdoc:identifier')?.value
						...
						if (child.value === 'dispatch') {
							...
							if (dispatchValue) {
								if (dispatchValue.type === 'mcdoc:struct') {
									...
									/* @ts-ignore */
									const key = `${_parent?.children?.[3]?.children?.[0].value}`.replace(/[\%\, ]/, '__') // should be sanitized enough
				...
				if (
					...
					/* @ts-ignore */
					if (attributes && attributes.length === 1 && !attributes[0].children && attributes[0].value === 'id') {
						/* @ts-ignore */
				...
				if (child.type !== 'error') return child
					...
					function range(
						...
						return `L${start?.line}${start?.col ? `:C${start?.col}` : ''
			...
			if (!args.dry && args.module) {
				const dir =
					parse(join(generated_path, 'module', resource)).dir
	...
	return [symbols, locales, errors] as [{ resource: string, children: AstNode[] }[], typeof locales, number]
}