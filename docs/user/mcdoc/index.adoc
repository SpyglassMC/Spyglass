...
When an index is not provided, it refers to the fallback case of the dispatcher, which will be used when a non-existing index is used in a <<s-dispatcher-type>>.
The default value of the fallback case is `any`.
