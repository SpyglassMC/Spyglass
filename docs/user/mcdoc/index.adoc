...
If, however, there exists a folder named `mcdoc` directly under the workspace and all `mcdoc` files inside the workspace are stored under of that direcotry, it will be considered as the root instead.
...
<<t-doc-comments>>:: <<t-doc-comment>>{star} // No regular comments are allowed between the individual <<t-doc-comment>>.
...
[[s-path-type,_PathType_]]
<<s-path-type>>:: <<t-path>> <<s-type-param-assignment>>^?^
