...
If, however, there exists a folder named `mcdoc` under the workspace and all `mcdoc` files are stored inside of the direcotry, it will be considered as the root instead.
...
// <<t-ident-start>>:: [`A`-`Z` `a`-`z` `_`]

[[t-ident-continue,IDENT_CONTINUE]]
<<t-ident-continue>>:: <<t-ident-start>> | U+`200C` | U+`200D` | (any character in the Unicode categories "`Non-Spacing Mark (`Mn`)`", "`Spacing Combining Mark (`Mc`)`", "`Decimal Digit Number (`Nd`)`", or "`Connector Punctuation (`Pc`)`")
// <<t-ident-continue>>:: <<t-ident-start>> | [`0`-`9`]
...
It can contain any Unicode letters and the underscore (`_`), but must not start with a digit.
...
NOTE: For SNBT validation, a strict mode, a loose mode, and a technically-compliant mode ("`te-co`" for short 🌮) are documented. +
The strict mode is used when an SNBT value is used as a predicate (e.g. in entity selectors, loot function conditions, etc.). +
The loose mode is used when an SNBT value is provided for deserialization (e.g. in `/setblock` or `/summon` commands).
Although some values are technically allowed for a given type (e.g. a compound tag means `false` when it's read as a boolean),
such cases are beyond a sane person's reasonable expectations and will not be allowed under the loose mode. +
The te-co mode, however, allows such bizarre values.
...
|SNBT (loose) |Any numeric values of `0` or `1` |-
|SNBT (te-co) |Any values |[[x-NbtCompound-getBoolean]] Per method `net/minecraft/nbt/NbtCompound#getBoolean` (yarn mappings), any NBT values can be read as a boolean. A non-numeric value is equivalent to `false`, while a numeric value is false if and only if it is a `0` after being converted to a byte number.
|===
...
|JSON |String values matching the length range |-
|SNBT (strict and loose) |String values matching the length range |-
|SNBT (te-co) |Any values that match the length range after being converted to a string |[[x-NbtElement-asString]] Per method `net/minecraft/nbt/NbtElement-asString` (yarn mappings), any NBT values can be read as a string. String tags have their string values as the result, while other tags have their string representation as the result.
|===
...
.Valid values for a literal boolean type
|===
|Format |Valid values

|JSON |`false` if the type is `false`; `true` is the type is `true`
|SNBT (strict) |`false` or `0b` if the type is `false`; `true` or `1b` if the type is `true`
|SNBT (loose) |Any numeric values of `0` if the type is `false`; any numeric values of `1` if the type is `true`
|SNBT (te-co) |Any values that match the type after being <<x-NbtCompound-getBoolean,converted to a boolean>>
|===
...
.Valid values for a literal string type
|===
|Format |Valid values

|JSON |String values that have the same value as the string literal
|SNBT (strict and loose) |String values that have the same value as the string literal
|SNBT (te-co) |Any values that when <<x-NbtElement-asString,converted to string>> is the same as the string literal
|===
...
.Valid values for a literal number type
|===
|Format |Valid values |Notes

|JSON |Number values that have the same numeric value as the number literal |JSON only has one number type, so the type part of a literal number type is not checked.
|SNBT (strict) |Number values that have the same numeric value and the same type as the number literal |-
|SNBT (loose) |Number values that have the same numeric value as the number literal |-
|SNBT (te-co) |Actually I gave up on documenting this mode. Good luck |-
|===
...
[[s-struct,_Struct_]]
<<s-struct>>:: <<s-prelim>> `struct` <<t-ident>>^?^ <<s-type-param-block>>^?^ <<s-struct-block>>
...
<<s-type-param>>:: <<t-ident>> (`extends` <<t-path>>)^?^
...
A pair of angle brackets (`<` and `>`) can be put after the struct identifier to declare **type parameters**, which should be passed in when the struct is referenced.
...
<<t-static-index-key>>:: `%fallback` | `%none` | <<t-ident>> | <<t-string>> | <<t-res-loc>>
...
It cannot be used in <<dispatch-statement,dispatch statements>>, as the fallback case is generated automatically and cannot be manually declared.
...
<<s-struct-inject>>::
	`struct` <<t-path>> <<s-type-param-block>>^?^ <<s-struct-block>> +
