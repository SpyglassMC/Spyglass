...
<<t-reserved-words>>:: `any` | `boolean` | `byte` | `double` | `enum` | `false` | `float` | `int` | `key` | `long` | `short` | `string` | `struct` | `super` | `true`
****
...
|JSON |String values |-
|SNBT (strict and loose) |String values |-
|SNBT (te-co) |Any values |[[x-NbtElement-asString]] Per method `net/minecraft/nbt/NbtElement-asString` (yarn mappings), any NBT values can be read as a string. String tags have their string values as the result, while other tags have their string representation as the result.
|===
...
The <<dispatcher-fallback-case,fallback case>> is used when the index is omitted.
